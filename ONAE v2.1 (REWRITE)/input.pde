...
void keyPressed(){
  ...
  if(current_state == state.game && !is_flasher_jumpscare){
    ...
    //camera toggle
    if((key == 's' || key == 'S') && !is_camera_cooldown){
      if(!edger_attack){
        edger_chance = int(random(0, 21));
      ...
      if(is_camera){
        ...
        flasher_chance = int(random(0, 21));
      }
    }
    //left door
    if((key == 'a' || key == 'A')){
      if(!is_door_cooldown && millis() - door_cooldown_start >= 1000){
        door.amp(volume);
        door.play();
        right_door_on = false;
    ...
    //right door
    if((key == 'd' || key == 'D')){
      if(!is_door_cooldown && millis() - door_cooldown_start >= 1500){
        door.amp(volume);
        door.play();
        left_door_on = false;
