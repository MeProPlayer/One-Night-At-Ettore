...
void mousePressed() {
  ...
  if (current_state == state.info && mouse_check(780, 1141, 823, 863)) {
...
void keyPressed() {
  ...
  // game input
  if ((current_state == state.game || current_state == state.tutorial) && !is_flasher_jumpscare) {
    // edger counter
    ...
    // camera toggle
    if (key == 's' || key == 'S') {
      if (!edger_attack && current_state != state.tutorial) {
